...
class GPTConfig:
    ...
    vocab_size: int = 50257 # number of tokens: 50000 BPE merges + 256 bytes tokens + 1 end token
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # not a bias, but OpenAI named it bias
...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        # start will all of the parameters
        param_dict = {pn: p for pn, p in self.named_parameters()}
        param_dict = {pn: p for pn, p in param_dict.items() if p.requires_grad}
        # any 2D parameter will be weight decayed
...
class DataLoader:
    ...
    def next_batch(self):
        ...
        # if loading of the next batch is out of bounds, reset
...
def main():
    ...
    def get_lr(iter):
        ...
        # min LR at the end of training
        if iter > max_steps:
            return min_lr
        # cosine decay
        decay_ratio = (iter - warmup_steps) / (max_steps - warmup_steps)
        assert 0 <= decay_ratio <= 1
        coeff = 0.5 * (1.0 + math.cos(math.pi * decay_ratio)) # starts at 1, ends at 0
    ...
    for iter in range(max_steps):
        ...
        if master_process:
            print(f"step {iter:4d} | loss: {loss_accum.item():.6f} | lr: {lr:.4e} | norm: {norm:.4f} | dt: {dt*1000:.2f}ms | tokens/sec: {tokens_per_second:.2f}")
